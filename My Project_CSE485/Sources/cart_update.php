...
{
    $ma_mon   = filter_var($_POST["product_id"], FILTER_SANITIZE_STRING); //product id
    ...
    //Giới hạn sản phẩm
    if($product_qty > 100){
        die('<div align="center">Vượt quá sản phẩm có thể đặt</div>');
    }
    
    //Lấy thông tin chi tiết sản phẩm bằng product_id
    $results = $mysqli->query("SELECT ten_mon, don_gia FROM food WHERE ma_mon='$ma_mon' LIMIT 1");
    $obj = $results->fetch_object();
    
    if ($results) { //Kiểm tra có dữ liệu hay không
        
        //Chuẩn bị array(mảng) để lưu thông tin sản phẩm 
        $new_product = array(array('name'=>$obj->ten_mon, 'id'=>$ma_mon, 'qty'=>$product_qty, 'price'=>$obj->don_gia));
